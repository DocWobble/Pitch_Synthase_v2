#!/usr/bin/env python3
"""Full 8-slide deck generation for the Newton-archetype test job, approach_2 ("Spend Gradient")."""
...
JOB_ID = "pd_ed26cfd043e14b31"
APPROACH_ID = "approach_2"  # "Spend Gradient"
SLIDE_COUNT = 8
...
async def main():
    ...
    print(f"Job: {JOB_ID}  Approach: {approach['label']}  Archetype: {archetype['label']}", file=sys.stderr, flush=True)
    print(f"excepted_inference_elements: {job.get('excepted_inference_elements')}", file=sys.stderr, flush=True)
    print(f"Generating {SLIDE_COUNT}-slide PAID deck...", file=sys.stderr, flush=True)
    ...
    job = db.get_job(JOB_ID)
    print(f"\nFinal status: {job['status']}", file=sys.stderr, flush=True)
    if job.get("error_message"):
        print("error:", job["error_message"], file=sys.stderr, flush=True)

    strategy_path = workers.job_dir(JOB_ID) / "strategy" / "paid_storyboard.json"
    if strategy_path.exists():
        import json
        doc = json.loads(strategy_path.read_text())
        print("\nauthorized_inferences decided by deck builder:", file=sys.stderr, flush=True)
        print(json.dumps(doc.get("authorized_inferences"), indent=2), file=sys.stderr, flush=True)
