...
#      model to INFER from its own latent training (comparable pitches,
#      products, business models), not from source material. Authorized
...
EXCEPTED_INFERENCE_ELEMENT_CHOICES = ("pricing",)


def _inference_exception_block(excepted_inference_elements: Sequence[str] | None) -> str:
    elements = [e for e in (excepted_inference_elements or []) if e]
    if not elements:
        return ""
    return dedent(
        f"""
        Authorized inference exception -- applies ONLY to these elements:
        {_json(elements)}
        For these elements only, the user has explicitly authorized you to
        INFER a plausible, well-reasoned structure based on your own latent
        knowledge of comparable successful pitches, products, buyers, and
        business models -- even though the source material does not state
        it. This is not a general license: every other claim, number, and
        proof point must still come strictly from elevator_pitch/doc_text as
        usual, and this exception does NOT relax any other rule -- internal
        consistency, spelling, and diagram correctness all still fully
        apply to inferred content exactly as they would to sourced content.
        Concretely: decide the inferred value for each excepted element
        exactly ONCE for this deck, and reuse that identical value verbatim
        everywhere it appears -- an inferred price, tier structure, or
        number is canon the moment it is decided, and every slide that
        touches it must agree with every other slide, the same as if it had
        come from the source. An inference under this exception must still
        read as a considered proposal (e.g. "a pricing structure sized for
        this buyer/segment might look like..."), not be dressed up as a
        sourced fact, but it is not license for each slide to invent its
        own different number independently.
        """
    ).strip()
...
def template_drafter_prompt_archetype_first(
    *,
    ...
    return dedent(
        ...
        authorize fabricated claims about traction, approvals, revenue, customers,
        ...
        - no invented traction, approval, customer, revenue, deployment, or validation claims;
...
def approach_drafter_prompt(
    *,
    ...
    optional supporting document), not just a one-sentence problem_need. Returns
    four full strategic pitch approaches as text only -- no images. Each approach
    names its own thesis and visual direction; the actual single-slide image
    prompt is written by a later, separate call once the user approves an approach.

    If selected_archetype is None, the drafter must pick the single best-fit
    archetype for the whole pitch from the fixed catalog and state which one it
    picked. All four approaches share one archetype -- only the strategic angle
    and visual direction diverge between them.
    """
    archetype_instruction = (
        f"The founder archetype is fixed by the user. Use exactly this archetype for "
        f"all four approaches -- do not substitute or re-derive it:\n{_json(selected_archetype)}"
        if selected_archetype is not None
        else (
            "No archetype was specified. Choose the single best-fit founder archetype for "
            "this pitch from the catalog below, and use that same archetype for all four "
            "approaches. State which one you chose in the output.\n\n"
            + "\n\n".join(
    ...
    exception_block = _inference_exception_block(excepted_inference_elements)
    exception_prefix = f"{exception_block}\n\n" if exception_block else ""

    return dedent(
        f"""
        {exception_prefix}You are drafting four competing strategic pitch approaches for the same
        real invention, for the same real audience, using the same founder archetype.

        This is a prompt-response experiment, not a full deck build. Nothing here
        generates images. You receive the actual elevator pitch -- not a placeholder --
        so ground every approach in the real mechanism and claims it describes. Do not
        invent a different mechanism than the one described, EXCEPT for any element
        named in the authorized inference exception above, if present.

        Generative frame:
        Four different competing teams pitched this exact invention to this exact
        audience. All four embodied the same founder archetype and the same intended
        impression, but each team discovered a different strategic emphasis and visual
        doctrine through which that archetype became credible and differentiating for
        this specific audience and mechanism.
        ...
        Required invention
        ------------------
        Invent exactly four strategic pitch approaches. Each approach must:
        - have a unique two- or three-word label;
        - not be a company name;
        - not repeat or paraphrase the archetype label;
        - describe a genuinely different strategic emphasis (which claims lead, which
          proof matters most, which fear or ambition it activates in this audience),
          not just a different color palette;
        - remain strictly faithful to the real mechanism in the elevator pitch --
          approaches may differ in emphasis and framing, never in the underlying facts,
          except for elements covered by the authorized inference exception above;
        - differ from the other three across: which claim leads, evidence posture,
          rhetorical temperature, and visual doctrine (artifact tradition, typography,
          material language, composition).

        Output contract
        ---------------
        Return strict JSON only. Top-level keys: "archetype" and "approaches".
        "archetype": the archetype object actually used (echo the fixed one, or the
        one you chose if none was given).
        "approaches": exactly four objects, approach_id values approach_1 through
        approach_4, each containing:
        - approach_id
        - label
        - pitch_angle: one to two sentences on the specific strategic emphasis this
          approach takes and why it wins for this audience;
        - key_differentiator: one sentence on what makes this approach distinct from
          the other three;
        - visual_direction: a compact description of this approach's artifact
          tradition, typography, material language, and composition -- enough detail
          that a later call could write one hero-slide image prompt from it alone,
          without needing anything else.

        JSON shape:
        {{
          "archetype": {{"archetype_id": "...", "label": "...", "role": "...", "posture": "...", "win_condition": "..."}},
          "approaches": [
            {{
...
def single_slide_drafter_prompt(
    *,
    ...
    (traction, customers, revenue, approvals) beyond what the elevator pitch/doc
    actually states, except for elements the user has explicitly excepted (see
    ...
    return dedent(
        f"""
        {exception_prefix}You are writing the single hero slide that best proves out one specific,
        ...
        customers, revenue, traction, approvals, partnerships, or deployments --
        except for elements covered by the authorized inference exception above,
        if present.
        ...
        - not fabricate traction, customers, revenue, approvals, partnerships, or deployments
...
def single_slide_image_prompt(*, slide_draft: Mapping[str, Any]) -> str:
    ...
    return dedent(
        ...
        - Do not fabricate traction, customer names, revenue, approvals, or
    ...
    *,
    ...
    if artifact_mode == "paid":
        return dedent(
            ...
            - The presentation has {len(numbers)} total slides, and one clean
...
def deck_builder_prompt(
    *,
    ...
    return dedent(
        f"""
        {exception_prefix}Follow this Deck Builder prompt exactly:
        {deck_generation_prompt}

        Reminder: this presentation was designed and produced by the founding
        team's own graphic designer / marketing lead, as described above -- you
        are reconstructing that person's actual finished slides, not generating
        an AI-made deck from nothing. Plan and describe each slide accordingly.
        ...
        - Do not invent traction, customers, revenue, approvals, partnerships,
          deployments, endorsements, or proof -- except for elements covered by
          the authorized inference exception above, if present, where a clearly
          labeled inferred proposal is permitted for that element only.
        ...
        - The image generator will receive this image_prompt, the spliced-in
          ...
          image.
        {"- If an authorized inference exception is in effect (see above), decide the value for each excepted element exactly ONCE in the top-level authorized_inferences object below, and reuse that identical decided value verbatim in every image_prompt that touches it -- do not let different slides independently invent different numbers/structures for the same excepted element." if exception_block else ""}
        ...
        {{
          ...
          "authorized_inferences": {{
            "<element name, e.g. pricing>": "<the ONE decided inferred value/structure for this element, stated once, reused verbatim everywhere it appears>"
          }},''' if exception_block else ""}
          "storyboard": [
            {{
...
def finalize_refiner_prompt(
    ...
    for spec in slide_specs:
        ...
        cr = (reviewed_slides.get(str(idx)) or {}).get("change_requests", "").strip()
...
def slide_source_accuracy_verifier_prompt(
    ...
    preamble_parts = [b for b in (exception_block, overrides_block) if b]
    ...
    return dedent(
        ...
        visible on this slide are consistent with the real source material
        below -- OR, for elements under an authorized inference exception,
        consistent with this deck's one decided value for that element. You
        do not judge general spelling of non-factual text or diagram
        correctness -- other specialists handle those.

        Source material (ground truth -- reference only):
        Elevator pitch:
        {elevator_pitch.strip()}
        {doc_block}{decided_inferences_block}
