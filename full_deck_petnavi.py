#!/usr/bin/env python3
"""
Take PetNavi's "Proof Stack" approach (strongest fit for an actual investor
audience: leads with evidence -- working architecture, real detected
hardware, modeled unit economics -- and reframes the raise as manufacturing/
certification capital rather than speculative R&D) to full PAID deck
generation. 16 slides, matching the real uploaded outline's length -- this
is also the first test past the old 10-slide self-serve cap, now raised
since it was never a technical ceiling (see prompts.py MAX_SELF_SERVE_SLIDE_COUNT).

Stops after generation_worker (status: awaiting_review) -- no finalization
...
JOB_ID = "pd_5072b5fa2fdb4ab2"  # PetNavi (existing-pitch test)
APPROACH_ID = "approach_3"  # "Proof Stack"
SLIDE_COUNT = 16
...
async def main():
    ...
    print(f"Generating {SLIDE_COUNT}-slide PAID deck (no finalization pass)...", file=sys.stderr)
